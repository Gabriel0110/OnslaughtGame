...
public class CameraController : MonoBehaviour
{
    ...
    private void Awake()
    {
        // Set desired camera on awaken
        transform.rotation = Quaternion.Euler(target.rotation.eulerAngles.x, 13, target.rotation.eulerAngles.z);
    }
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButton(1))
        {
            if (UIController.instance.GetComponent<CheckClicks>().getRaycastResults().Count == 0)
            {
                ...
                if (PlayerController.instance.GetComponent<PlayerManager>().isPlayerAlive() && !PlayerController.instance.GetComponent<PlayerManager>().isPlayerStunned())
                {
    ...
    void LateUpdate()
    {
        ...
        // calculate the desired distance for scrolling
        if (Input.GetAxis("Mouse ScrollWheel") != 0f)
        {
            if (!UIController.instance.GetComponent<CheckClicks>().IsPointerOverUIObject())
            {
